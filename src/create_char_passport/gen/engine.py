...
def _friendly_error(exc: BaseException) -> str:
    ...
    if (
        ...
        return (
            "Слишком часто — сработал поминутный лимит Gemini на генерацию картинок "
            "(RPM), не нехватка денег. Подождите ~минуту и генерируйте по одному кадру. "
            "Если повторяется и при редких запросах — проверьте квоту/биллинг ключа."
        )
